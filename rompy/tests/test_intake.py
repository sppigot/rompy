...
def test_intake_local_stack():
    ...
    mycat = intake.open_catalog(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "catalog.yaml")
    )
...
def test_intake_remote_stack():
    ...
    ds = rompy.cat.bom.wavewatch3_nci(
        hindcast=False,
        fmt_fields={
            "fcdate": list(
                pd.date_range("2021-01-21", "2021-01-22").strftime("%Y%m%d")
            ),
            "hr": ["0000"],
            "grid": ["PER"],
            "output": ["msh"],
...
def test_intake_aodn_wave_buoys_remote():
    ...
    df = rompy.cat.aodn.nrt_wave_buoys(
        startdt="2021-04-05",
        enddt="2021-04-29",
...
def test_intake_aodn_altimetry_points_remote():
    ...
    df = rompy.cat.aodn.nrt_wave_altimetry_points(
        startdt="2020-02-03",
        enddt="2020-02-04",
        geom="POLYGON ((111.0000000000000000 -33.0000000000000000, 111.0000000000000000 -31.5000000000000000, 115.8000030517578125 -31.5000000000000000, 115.8000030517578125 -33.0000000000000000, 111.0000000000000000 -33.0000000000000000))",
    ).read()
...
def test_intake_aodn_altimetry_remote_stack():
    ...
    ds = rompy.cat.aodn.nrt_wave_altimetry(
        startdt="2020-02-03",
        enddt="2020-02-04",
        geom="POLYGON ((111.0000000000000000 -33.0000000000000000, 111.0000000000000000 -31.5000000000000000, 115.8000030517578125 -31.5000000000000000, 115.8000030517578125 -33.0000000000000000, 111.0000000000000000 -33.0000000000000000))",
        ds_filters={"subset": {"data_vars": ["SWH_C"]}},
    ).to_dask()
...
def test_intake_aodn_sar_points_remote():
    ...
    df = rompy.cat.aodn.nrt_wave_sar_points(
        startdt="2021-02-01",
        enddt="2021-04-29",
        geom="POLYGON ((111.0000000000000000 -33.0000000000000000, 111.0000000000000000 -31.5000000000000000, 115.8000030517578125 -31.5000000000000000, 115.8000030517578125 -33.0000000000000000, 111.0000000000000000 -33.0000000000000000))",
    ).read()
...
def test_intake_aodn_sar_remote_stack():
    ...
    ds = rompy.cat.aodn.nrt_wave_sar(
        startdt="2021-03-01",
        enddt="2021-04-29",
        geom="POLYGON ((111.0000000000000000 -33.0000000000000000, 111.0000000000000000 -31.5000000000000000, 115.8000030517578125 -31.5000000000000000, 115.8000030517578125 -33.0000000000000000, 111.0000000000000000 -33.0000000000000000))",
        ds_filters={"subset": {"data_vars": ["HS_PART"]}},
    ).to_dask()
