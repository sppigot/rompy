...
class HOTFILE(NamelistBaseModel):
    ...
    BEGTC: str = Field("'19980901.000000'", description="Starting time of hotfile writing")
...
class PROC(NamelistBaseModel):
    ...
    LNAUTOUT: str = Field('T', description="Nautical output of all quantities in degrees")
    BEGTC: str = Field("'19980901.000000'", description="Time for start the simulation, ex:yyyymmdd. hhmmss")
    ...
    ENDTC: str = Field("'19980901.060000'", description="Time for stop the simulation, ex:yyyymmdd. hhmmss")
...
class GRID(NamelistBaseModel):
    ...
    IGRIDTYPE: int = Field(3, description="Gridtype used. 1 ~ XFN, 2 ~ WWM-PERIODIC, 3 ~ SCHISM, 4 ~ OLD WWM GRID")
...
class BOUC(NamelistBaseModel):
    ...
    LBINTER: str = Field('F', description="Do interpolation in time if LBCSE=T (not available for quasi-steady mode within the subtime steps)")
    ...
    LPARMDIR: str = Field('F', description="If LPARMDIR is true than directional spreading is read from WBDS and must be in exponential format at this time, only valid for 1d Spectra")
    FILEWAVE: str = Field("'wwmbnd.gr3'", description="Boundary file including discrete wave spectra")
    ...
    BEGTC: str = Field("'19980901.000000'", description="Begin time of the wave boundary file (FILEWAVE)")
    ...
    FILEBOUND: str = Field("'wwmbnd.gr3'", description="Boundary file defining boundary and Neumann nodes.")
    IBOUNDFORMAT: int = Field(1, description="1 ~ WWM, 3 ~ WW3 (2D spectra in netcdf format only - LBCWA=T).")
    WBHS: float = Field(4.0, description="Hs at the boundary for parametric spectra")
    WBSS: float = Field(2.0, description="1 or -1: Pierson-Moskowitz, 2 or -2: JONSWAP, 3 or -3: all in one BIN,")
    ...
    WBDSMS: float = Field(1.0, description="Directional spreading value in degrees (1) or as exponent (2)")
    WBDS: float = Field(20.0, description="Directional spreading at the boundary (degrees/exponent)")
    ...
    NCDF_HS_NAME: str = Field("'hs'", description="NETCDF var. name for the significant wave height (normally it is just 'hs')")
    NCDF_DIR_NAME: str = Field("'dir'", description="NETCDF var. name for the mean wave direction (normally it is just 'dir')")
    NCDF_SPR_NAME: str = Field("'spr'", description="NETCDF var. name for the mean directional spreading (normally it is just 'spr')")
    NCDF_FP_NAME: str = Field("'fp'", description="NETCDF var. name for the peak freq. (normally it is just 'fp')")
    NCDF_F02_NAME: str = Field("'t02'", description="NETCDF var. name for the zero down crossing freq. (normally it is just 't02')")
...
class NUMS(NamelistBaseModel):
    ...
    LSOUBOUND: str = Field('F', description="Source Terms on wet land/island boundary. Use T if SMETHOD=6")
    ...
    NQSITER: int = Field(10, description="# of quasi-steady (Q-S) sub-divisions within each WWM time step (trial and errors)")
    ...
    QSCONV4: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill avg. rel. wave period criteria EPSH4")
    QSCONV5: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill avg. rel. wave steepness criteria EPSH5")
    ...
    LCHKCONV: str = Field('T', description="needs to set to .true. for quasi-steady mode. in order to compute the QSCONVi criteria and check them")
...
class HISTORY(NamelistBaseModel):
    BEGTC: str = Field("'19980901.000000'", description="Start output time, yyyymmdd. hhmmss;")
    ...
    ENDTC: str = Field("'19980910.000000'", description="Stop time output, yyyymmdd. hhmmss")
    ...
    USE_SINGLE_OUT: str = Field('T', description="T: Use single precision in the output of model variables (default)")
    PARAMWRITE: str = Field('T', description="T/F: Write the physical parametrization and")
...
class STATION(NamelistBaseModel):
    BEGTC: str = Field("'19980901.000000'", description="Start simulation time, yyyymmdd. hhmmss; must fit the simulation time otherwise no output")
    DELTC: list = Field([1, ''], description="Time step for output; if smaller than simulation time step, the latter is used (output every step for better 1D 2D spectra analysis)")
    UNITC: str = Field("'SEC'", description="Unit")
    ENDTC: str = Field("'20081101.000000'", description="Stop time simulation, yyyymmdd. hhmmss")
    OUTSTYLE: str = Field("'STE'", description="output option - use 'NO' to maximize efficiency during parallel run when using MPI")
    ...
    CUTOFF: str = Field('8*0.44', description="cutoff freq (Hz) for each station - consistent with buoys")
...
class PETSCOPTIONS(NamelistBaseModel):
    ...
    INITIALGUESSNONZERO: str = Field('T', description="Tells the iterative solver that the initial guess is nonzero; otherwise KSP assumes the initial guess is to be zero")
