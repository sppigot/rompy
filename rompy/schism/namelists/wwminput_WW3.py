...
class PROC(NamelistBaseModel):
    ...
    DIMMODE: int = Field(2, description="Mode of run (ex: 1 = 1D, 2 = 2D) always 2D when coupled to SCHISM")
    LSTEA: str = Field('F', description="steady mode; under development")
    LQSTEA: str = Field('F', description="Quasi-Steady Mode; In this case WWM-II is doing subiterations defined as DELTC/NQSITER unless QSCONVI is not reached")
    LSPHE: str = Field('T', description="Spherical coordinates (lon/lat)")
    LNAUTIN: str = Field('T', description="Nautical convention for all inputs given in degrees (suggestion: T)")
    LNAUTOUT: str = Field('T', description="Output in Nautical convention")
    LMONO_IN: str = Field('F', description="For prescribing monochromatic wave height Hmono as a boundary conditions; incident wave is defined as monochromatic wave height, which is Hmono = sqrt(2) * Hs")
    LMONO_OUT: str = Field('F', description="Output wave heights in terms of Lmono")
    BEGTC: str = Field("'20030908.000000'", description="Time for start the simulation, ex:yyyymmdd. hhmmss")
    DELTC: int = Field(600, description="Time step (MUST match dt*nstep_wwm in SCHISM!)")
    UNITC: str = Field("'SEC'", description="Unity of time step")
    ENDTC: str = Field("'20031008.000000'", description="Time for stop the simulation, ex:yyyymmdd. hhmmss")
    DMIN: float = Field(0.01, description="Minimum water depth. This must be the same as h0 in SCHISM")

class COUPL(NamelistBaseModel):
    LCPL: str = Field('T', description="Couple with current model ... main switch - keep it on for SCHISM-WWM")
    RADFLAG: str = Field("'LON'", description="LON: Longuet-Higgin; VOR: vortex formulation")
    LETOT: str = Field('F', description="Option to compute the wave induced radiation stress. If .T. the radiation stress is based on the integrated wave spectrum")
    NLVT: int = Field(10, description="Number of vertical Layers; not used with SCHISM")
    DTCOUP: float = Field(600.0, description="Couple time step - not used when coupled to SCHISM")

class GRID(NamelistBaseModel):
    LCIRD: str = Field('T', description="Full circle in directional space")
    LSTAG: str = Field('F', description="Stagger directional bins with a half Dtheta; may use T only for regular grid to avoid char. line aligning with grid line")
    MINDIR: float = Field(0.0, description="Minimum direction for simulation (unit: degrees; nautical convention; 0: from N; 90: from E); not used if LCIRD = .T.")
    MAXDIR: float = Field(360.0, description="Maximum direction for simulation (unit: degrees); may be < MINDIR; not used if LCIRD = .T.")
    MDC: int = Field(36, description="Number of directional bins")
    FRLOW: float = Field(0.04, description="Low frequency limit of the discrete wave period (Hz; 1/period)")
    FRHIGH: float = Field(1.0, description="High frequency limit of the discrete wave period.")
    MSC: int = Field(36, description="Number of frequency bins")
    FILEGRID: str = Field("'hgrid_WWM.gr3'", description="Name of the grid file. hgridi_WWM.gr3 if IGRIDTYPE = 3 (SCHISM)")
    ...
    LVAR1D: str = Field('F', description="For 1d-mode if variable dx is used; not used with SCHISM")
    LOPTSIG: str = Field('F', description="Use optimal distributions of freq. in spectral space ... fi+1 = fi * 1.1. Take care what you high freq. limit is!")

class INIT(NamelistBaseModel):
    FILEHOT_IN: str = Field("'hotfile_in_WWM.nc'", description="(Full) hot file name for input (which can be copied from FILEHOT_OUT above)")
...
class HOTFILE(NamelistBaseModel):
    ...
    BEGTC: str = Field("'20030908.000000'", description="Starting time of hotfile writing. With ihot!=0 in SCHISM,")
    DELTC: float = Field(86400.0, description="time between hotfile writes")
    UNITC: str = Field("'SEC'", description="unit used above")
    ENDTC: str = Field("'20031008.000000'", description="Ending time of hotfile writing (adjust with BEGTC)")
    ...
    MULTIPLEOUT: int = Field(0, description="0: hotfile in a single file (binary or netcdf)")

class BOUC(NamelistBaseModel):
    LBCSE: str = Field('T', description="The wave boundary data is time dependent")
    LBINTER: str = Field('T', description="Do interpolation in time if LBCSE=T (not available for quasi-steady mode within the subtime steps)")
    LBCWA: str = Field('T', description="Parametric Wave Spectra")
    LBCSP: str = Field('F', description="Specify (non-parametric) wave spectra, specified in 'FILEWAVE' below")
    LINHOM: str = Field('T', description="Non-uniform wave b.c. in space")
    LBSP1D: str = Field('F', description="1D (freq. space only) format for FILEWAVE if LBCSP=T and LINHOM=F")
    LBSP2D: str = Field('F', description="2D format for FILEWAVE if LBCSP=T and LINHOM=F")
    BEGTC: str = Field("'20030908.000000'", description="Begin time of the wave boundary file (FILEWAVE)")
    ...
    FILEBOUND: str = Field("'wwmbnd.gr3'", description="Boundary file defining boundary conditions and Neumann nodes.")
    IBOUNDFORMAT: int = Field(3, description="1 ~ WWM, 3 ~ WW3 (2D spectra in netcdf format only - LBCWA=T).")
    FILEWAVE: str = Field("'bndfiles.dat'", description="Boundary file defining boundary input from WW3")
    LINDSPRDEG: str = Field('F', description="If 1-d wave spectra are read this flag defines whether the input for the directional spreading is in degrees (true) or exponent (false)")
    LPARMDIR: str = Field('F', description="If LPARMDIR is true then directional spreading is read from WBDS and must be in exponential format at this time, only valid for 1d Spectra")
    WBHS: float = Field(2.0, description="Hs at the boundary for parametric spectra")
    WBSS: int = Field(2, description="1 or -1: Pierson-Moskowitz, 2 or -2: JONSWAP, 3 or -3: all in one BIN,")
    WBTP: float = Field(8.0, description="Tp at the boundary (sec); mean or peak depending on the sign of WBSS")
    WBDM: float = Field(90.0, description="Avg. Wave Direction at the boundary")
    WBDSMS: int = Field(1, description="Directional spreading value in degrees (1) or as exponent (2)")
    WBDS: float = Field(10.0, description="Directional spreading at the boundary (degrees/exponent)")
    WBGAUSS: float = Field(0.1, description="factor for gaussian distribution if WBSS=1")
    WBPKEN: float = Field(3.3, description="Peak enhancement factor for Jonswap Spectra if WBSS=2")

class ENGS(NamelistBaseModel):
    MESNL: int = Field(1, description="Nonlinear Interaction NL4 , 1 ~ on, 0 ~ off (Discrete Interaction approx.)")
    MESIN: int = Field(1, description="Wind input: Ardhuin et al. (1) (use LSOURCESWAM = F);")
    IFRIC: int = Field(1, description="Formulation for atmospheric boundary layer, (IFRIC = 1 for MESIN = 1, IFRIC = 4 for MESIN=3);")
    MESBF: int = Field(1, description="Bottom friction: 1 - JONSWAP (Default); 2 - Madsen et al. (1989); 3 - SHOWEX")
    FRICC: float = Field(0.067, description="if MESBF=1: JONSWAP bottom friction coefficient [0.038,0.067]. If MESBF=2: physical bottom roughness (ignored if given in rough.gr3). If MESBF=3: D50 (if negative read from SHOWEX_D50.gr3)")
    MESBR: int = Field(1, description="Shallow water wave breaking; 0: off; 1: on")
    IBREAK: int = Field(1, description="Wave breaking formulation: 1 - Battjes and Janssen (1978)")
    ICRIT: int = Field(1, description="Wave breaking criterion: 1   - Constant breaker index (gamma) or gamma_TG defined with BRCR")
    BRCR: float = Field(0.78, description="either gamma, default is 0.73 for IBREAK=1,5 or gamma_TG, default is 0.42 for IBREAK=2,3 or biphase_ref, default is -4pi/9 = -1.3963 for IBREAK=4")
    ...
    a_BIPH: float = Field(0.2, description="Biphase coefficient, default 0.2 (intended for IBREAK=3)")
    BR_COEF_METHOD: int = Field(1, description="Method for the breaking coefficient: 1 - constant, 2 - adaptive")
    B_ALP: float = Field(0.5, description="breaking coefficient. If BR_COEF_METHOD = 2, B_ALP ~ 40")
    ZPROF_BREAK: int = Field(2, description="Vertical distribution function of wave breaking source term, only used in 3D run")
    BC_BREAK: int = Field(1, description="Apply depth-limited breaking at the boundaries: 1 - On; 0 - Off")
    IROLLER: int = Field(0, description="Wave roller model (e.g., see Uchiyama et al., 2010): 1 - On; 0 - Off; not used at the moment")
    ALPROL: float = Field(0.85, description="Alpha coefficient for the wave roller model (between 0 and 1): 1 - full conversion; 0 - no energy transferred to the roller")
    MEVEG: int = Field(0, description="Vegetation on/off. If on, isav must = 1 in param.nml")
    LMAXETOT: str = Field('T', description="Limit shallow water wave height by wave breaking limiter (default=T)")
    MESDS: int = Field(1, description="Formulation for the whitecapping source function; same value as MESIN")
    MESTR: int = Field(1, description="Formulation for the triad 3 wave interactions (MESTR = 0 (off), MESTR = 1 (Lumped Triad Approx. (LTA)), MESTR = 2 (corrected version of LTA by Salmon et al. (2016)))")
    TRICO: float = Field(0.1, description="proportionality const. (\alpha_EB); default is 0.1")
    TRIRA: float = Field(2.5, description="ratio of max. freq. considered in triads over mean freq.; 2.5 is suggested")
    TRIURS: float = Field(0.1, description="critical Ursell number; if Ursell # < TRIURS; triads are not computed")
...
class NUMS(NamelistBaseModel):
    ...
    RTHETA: float = Field(0.5, description="Weighing factor for DMETHOD = 1, not really useful since Crank Nicholson integration can only be monotone for CFL .le. 2")
    LITERSPLIT: str = Field('F', description="T: double Strang split; F: simple split (more efficient). Default: F")
    LFILTERTH: str = Field('F', description="")
    MAXCFLTH: float = Field(1.0, description="Max Cfl in Theta space; used only if LFILTERTH=T")
    FMETHOD: int = Field(1, description="")
    LFILTERSIG: str = Field('F', description="Limit the advection velocity in freq. space (usually F)")
    MAXCFLSIG: float = Field(1.0, description="Max Cfl in freq. space; used only if LFILTERSIG=T")
    LLIMT: str = Field('T', description="Switch on/off Action limiter, Action limiter must mostly be turned on.")
    MELIM: int = Field(1, description="Formulation for the action limiter")
    LIMFAK: float = Field(0.1, description="Proportionality coefficient for the action limiter MAX_DAC_DT = Limfak * Limiter; see notes above for value")
    LDIFR: str = Field('F', description="Use phase decoupled diffraction approximation according to Holthuijsen et al. (2003) (usually T; if crash, use F)")
    IDIFFR: int = Field(1, description="Extended WAE accounting for higher order effects WAE becomes nonlinear; 1: Holthuijsen et al. ; 2: Liau et al. ; 3: Toledo et al. (in preparation)")
    LCONV: str = Field('F', description="Estimate convergence criterian and write disk (quasi-steady - qstea.out)")
    LCFL: str = Field('F', description="Write out CFL numbers; use F to save time")
    NQSITER: int = Field(1, description="# of quasi-steady (Q-S) sub-divisions within each WWM time step (trial and errors)")
    QSCONV1: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill abs. wave height criteria EPSH1")
    QSCONV2: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill rel. wave height criteria EPSH2")
    QSCONV3: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill sum. rel. wave action criteria EPSH3")
    QSCONV4: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill rel. avg. wave steepness criteria EPSH4")
    QSCONV5: float = Field(0.98, description="Number of grid points [%/100] that have to fulfill avg. rel. wave period criteria EPSH5")
    LEXPIMP: str = Field('F', description="Use implicit schemes for freq. lower than given below by FREQEXP; used only if ICOMP=0")
    FREQEXP: float = Field(0.1, description="Minimum frequency for explicit schemes; only used if LEXPIMP=T and ICOMP=0")
    EPSH1: float = Field(0.01, description="Convergence criteria for rel. wave height ! EPSH1 < CONVK1 = REAL(ABS(HSOLD(IP)-HS2)/HS2)")
    EPSH2: float = Field(0.01, description="Convergence criteria for abs. wave height ! EPSH2 < CONVK2 = REAL(ABS(HS2-HSOLD(IP)))")
    EPSH3: float = Field(0.01, description="Convergence criteria for the rel. sum of wave action ! EPSH3 < CONVK3 = REAL(ABS(SUMACOLD(IP)-SUMAC)/SUMAC)")
    EPSH4: float = Field(0.01, description="Convergence criteria for the rel. avg. wave steepness criteria ! EPSH4 < CONVK4 = REAL(ABS(KHS2-KHSOLD(IP))/KHSOLD(IP))")
    EPSH5: float = Field(0.01, description="Convergence criteria for the rel. avg. waveperiod ! EPSH5 < REAL(ABS(TM02-TM02OLD(IP))/TM02OLD(IP))")
    LVECTOR: str = Field('F', description="Use optmized propagation routines for large high performance computers e.g. at least more than 128 CPU. Try LVECTOR=F first.")
    IVECTOR: int = Field(2, description="USed if LVECTOR=T; Different flavours of communications")
    LADVTEST: str = Field('F', description="for testing the advection schemes, testcase will be added soon")
    LCHKCONV: str = Field('F', description="needs to set to .true. for quasi-steady mode. in order to compute the QSCONVi criteria and check them")
    DTMIN_DYN: float = Field(1.0, description="min. time step (sec?) for dynamic integration, this controls in SMETHOD the smallest time step for the triads, DT = 1.s is found to work well.")
    NDYNITER: list = Field([100, ''], description="max. iteration for dyn. scheme afterwards the limiter is applied in the last step, for SMETHOD .eq. this controls the integration of the triad interaction terms, which is done dynamically.")
    DTMIN_SIN: float = Field(1.0, description="min. time steps for the full fractional step method, where each source term is integrated with its own fractional step")
    ...
    NDYNITER_SIN: list = Field([10, ''], description="max. iterations for each source term in the fractional step approach.")
    ...
    LSOUBOUND: str = Field('F', description="Do source terms on boundary, this is possible as long only dissipative processes are governing the spectral evolution, otherwise, with wind u will get the max. possible wave height")
    WAE_SOLVERTHR: list = Field(['1.e-6', ''], description="Threshold for the Block-Jacobi or Block-Gauss-Seider solver")
    MAXITER: list = Field([1000, ''], description="Max. number of iterations")
    PMIN: list = Field([1.0, ''], description="Max. percentage of non-converged grid points")
    LNANINFCHK: list = Field(['F', ''], description="Check for NaN and INF; usually turned off for efficiency")
    LZETA_SETUP: list = Field(['F', ''], description="Compute wave setup (simple momentum eq.)")
    ZETA_METH: list = Field([0, ''], description="Method for wave setup, Mathieu please explain!")
    LSOURCESWAM: list = Field(['F', ''], description="Use ECMWF WAM formualtion for deep water physics.")
    BLOCK_GAUSS_SEIDEL: list = Field(['T', ''], description="Use the Gauss Seidel on each")
    LNONL: str = Field('F', description="Solve the nonlinear system using simpler algorithm (Patankar)")
    ASPAR_LOCAL_LEVEL: int = Field(0, description="Aspar locality level")
    L_SOLVER_NORM: str = Field('F', description="Compute solver norm ||A*x-b|| as termination")
    LACCEL: str = Field('F', description="")

class HISTORY(NamelistBaseModel):
    BEGTC: str = Field("'20030908.000000'", description="Start output time, yyyymmdd. hhmmss;")
    DELTC: int = Field(1, description="Time step for output; if smaller than simulation time step, the latter is used (output every step for better 1D 2D spectra analysis)")
    UNITC: str = Field("'SEC'", description="Unit")
    ENDTC: str = Field("'20031008.000000'", description="Stop time output, yyyymmdd. hhmmss")
    ...
    GRIDWRITE: str = Field('T', description="T/F: Write the grid in the netcdf history file (default T)")
...
class STATION(NamelistBaseModel):
    BEGTC: str = Field("'20030908.000000'", description="Start simulation time, yyyymmdd. hhmmss; must fit the simulation time otherwise no output")
    DELTC: int = Field(600, description="Time step for output; if smaller than simulation time step, the latter is used (output every step for better 1D 2D spectra analysis)")
    UNITC: str = Field("'SEC'", description="Unit")
    ENDTC: str = Field("'20031008.000000'", description="Stop time simulation, yyyymmdd. hhmmss")
    ...
    NOUTS: list = Field(["'P-1'", "'P-2'", "'P-3'", "'P-4'", "'P-5'", "'P-6'", "'P-7'", "'P-8'", "'P-9'", "'P-10'", "'P-11'", "'P-12'", "'P-13'", "'P-14'", "'P-15'"], description="")
    XOUTS: list = Field([-76.046, -76.778, -75.81, -75.72, -74.842, ''], description="")
    YOUTS: list = Field([39.152, 38.556, 38.033, 37.551, ''], description="")
    CUTOFF: str = Field('15*0.44', description="cutoff freq (Hz) for each station - consistent with buoys")
    LSP1D: str = Field('T', description="1D spectral station output")
    LSP2D: str = Field('F', description="2D spectral station output")
    LSIGMAX: str = Field('T', description="Adjust the cut-freq. for the output (e.g. consistent with buoy cut-off freq.)")
...
class PETSCOPTIONS(NamelistBaseModel):
    ...
    RTOL: str = Field('1.E-20', description="the relative convergence tolerance (relative decrease in the residual norm)")
    ABSTOL: str = Field('1.E-20', description="the absolute convergence tolerance (absolute size of the residual norm)")
    ...
    INITIALGUESSNONZERO: str = Field('F', description="Tells the iterative solver that the initial guess is nonzero; otherwise KSP assumes the initial guess is to be zero")
    GMRESPREALLOCATE: str = Field('T', description="Causes GMRES and FGMRES to preallocate all its needed work vectors at initial setup rather than the default, which is to allocate them in chunks when needed.")
