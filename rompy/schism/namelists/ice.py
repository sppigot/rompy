...
class ICE_IN(NamelistBaseModel):
    ...
    ice_cutoff: str = Field('1.e-3', description="cut-off thickness [m] or fraction for ice. No ice velocity if *<=ice_cuttoff")
    evp_rheol_steps: int = Field(200, description="the number of sybcycling steps in EVP")
    mevp_rheol_steps: int = Field(200, description="the number of iterations in mEVP")
    ice_atmos_stress_form: int = Field(1, description="")
    cdwin0: str = Field('2.e-3', description="needed if ice_atmos_stress_form=0 (const Cdw)")
    delta_min: str = Field('2.0e-9', description="(1/s) Limit for minimum divergence (Hibler, Hunke")
    theta_io: float = Field(0.0, description="ice/ocean rotation angle. [degr]. Usually 0 unless vgrid is too coarse")
    mevp_coef: int = Field(0, description="")
    mevp_alpha1: float = Field(200.0, description="const used in mEVP (constitutive eq), if mevp_coef=0")
    mevp_alpha2: float = Field(200.0, description="const used in mEVP for momentum eq, if mevp_coef=0")
    ...
    ice_gamma_fct: float = Field(0.25, description="smoothing parameter; 1 for max positivity preserving")
    ...
    lead_closing: float = Field(0.5, description="lead closing parameter [m] - larger values slow down freezing-up but increase sea ice thickness")
    Saterm: float = Field(0.5, description="Semter const -smaller value could slow down melting")
