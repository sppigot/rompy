...
class ICE_IN(NamelistBaseModel):
    ...
    ievp: int = Field(2)
    ice_cutoff: str = Field('1.e-3')
    evp_rheol_steps: int = Field(200)
    mevp_rheol_steps: int = Field(200)
    ice_atmos_stress_form: int = Field(1)
    cdwin0: str = Field('2.e-3')
    delta_min: str = Field('2.0e-9')
    theta_io: float = Field(0.0)
    mevp_coef: int = Field(0)
    mevp_alpha1: float = Field(200.0)
    mevp_alpha2: float = Field(200.0)
    mevp_alpha3: float = Field(200.0)
    mevp_alpha4: str = Field('2.e-2')
    pstar: float = Field(15000.0)
    ellipse: float = Field(2.0)
    c_pressure: float = Field(20.0)
    ncyc_fct: int = Field(1)
    niter_fct: int = Field(3)
    ice_gamma_fct: float = Field(0.25)
    depth_ice_fct: float = Field(5.0)
    h_ml0: float = Field(0.1)
    salt_ice: float = Field(5.0)
    salt_water: float = Field(34.0)
    lead_closing: float = Field(0.5)
    Saterm: float = Field(0.5)
    albsn: float = Field(0.85)
    albsnm: float = Field(0.75)
