...
class MICE_IN(NamelistBaseModel):
    ...
    ihot_mice: int = Field(1, description="0: cold start 1: restart 2: hotstart_from_HYCOM")
    ice_advection: int = Field(6, description="advection on/off 3: upwind 4: center-difference 5: tvd 6: tvd-up 7: TVD_Casulli")
    ice_therm_on: int = Field(1, description="ice thermodynamics on/off flag")
    ievp: int = Field(2, description="1: EVP; 2: mEVP")
    ice_cutoff: float = Field(0.001, description="cut-off thickness [m] or fraction for ice. No ice velocity if *<=ice_cuttoff")
    evp_rheol_steps: int = Field(500, description="the number of sybcycling steps in EVP")
    mevp_rheol_steps: int = Field(500, description="the number of iterations in mEVP")
    delta_min: str = Field('1.0e-11', description="(1/s) Limit for minimum divergence (Hibler, Hunke")
    theta_io: float = Field(0.0, description="ice/ocean rotation angle. [degr]")
    mevp_alpha1: float = Field(200.0, description="const used in mEVP (constitutive eq)")
