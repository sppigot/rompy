...
class SwanModel(BaseModel):
    ...
    def _get_grid(self, key=None):
        ...
        if key is not None and key in self.settings.keys():
            ...
            grid_spec = self._default_context["bottom_grid"]
...
class SwanGrid(BaseGrid):
    def __init__(
        ...
        if any([self.__dict__["_SwanGrid__" + k] is None for k in mandatory_args]):
            raise ValueError(
                f"SwanGrid object of type = {gridtype} require values for the following arguments "
                + str(mandatory_args)
            )
    ...
    def __gen_reg_cgrid(self):
        # Grid at origin
        i = np.arange(0.0, self.dx * (self.nx + 0.1), self.dx)
        j = np.arange(0.0, self.dy * (self.ny + 0.1), self.dy)
